...
class ProsDetailViewController: BaseViewController , ProHeaderDelegate , UITableViewDelegate, UITableViewDataSource {
    ...
    func saveExitDate(notification : NSNotification) {
        ...
        timer2 = NSTimer.scheduledTimerWithTimeInterval(2, target: self, selector: #selector(ProsDetailViewController.checkDate), userInfo: nil, repeats: true)
    ...
    func startTimer(notification : NSNotification) {
        if self.isViewLoaded() && (self.view.window != nil) {
            ...
            timer = NSTimer.scheduledTimerWithTimeInterval(2, target: self, selector: #selector(ProsDetailViewController.checkDateToShowMaincontroller), userInfo: nil, repeats: true)
            print(timer)
        }
